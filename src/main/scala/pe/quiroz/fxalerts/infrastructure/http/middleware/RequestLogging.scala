 ...
 *   - registra una única línea con método, ruta, código de estado y duración.
 *
 * No se registran cuerpos, cabeceras ni parámetros de consulta: solo la ruta, que en esta API no
 * transporta datos de negocio más allá de identificadores. Un identificador recibido se acepta
 * únicamente si es corto y alfanumérico, para que nunca llegue al log texto arbitrario del cliente.
 */
object RequestLogging:
  ...
  def apply[F[_]: Async: Logger](app: HttpApp[F]): HttpApp[F] =
    Kleisli { request =>
      for
        ...
        _ <- result match
          case Right(response) =>
            logLine(request, requestId, response.status.code, elapsed)
          case Left(error) =>
            Logger[F].error(error)(line(request, requestId, 500, elapsed))
  ...
  private def logLine[F[_]: Logger](
    ...
    val message = line(request, requestId, status, elapsed)
